...
rawQRS = QRS;
QRS = clean_peak_train(QRS, EEG1.srate, 0.45, 2.4);
fprintf('Cleaned fMRIB QRS peaks: %d raw -> %d retained\n', numel(rawQRS), numel(QRS));
evt = EEG1.event;
keepEvt = true(1, numel(evt));
for ii = 1:numel(evt)
    keepEvt(ii) = ~strcmp(evt(ii).type, 'qrs');
end
EEG1.event = evt(keepEvt);
EEG1.event = AddMarker(EEG1.event, QRS, 1, 0, 'qrs', 'Response');
EEG1.event = sortlatency(EEG1.event, 1);
...
pkloc = clean_peak_train(pkloc, EEG2.APPEAR.Fs, 0.45, 2.4);
...
badInter              =[];
[EEG_poten, badInter_poten]       = pop_rejcont(EEG, 'elecrange',1:EEG.nbchan ,...
...
[EEG.data, ~] = repair_nonfinite_eeg(EEG.data);
[EEG_tmp.data, ~] = repair_nonfinite_eeg(EEG_tmp.data);
